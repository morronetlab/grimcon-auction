...
let primaryImages = ["https://images.unsplash.com/photo-1575936123452-b67c3203c357", "https://images.unsplash.com/photo-1575936123452-b67c3203c357"];
  ...
  if (endTimes[i] - nowTime < 0) {
    ...
    liveRef.get().then(function (doc) {
      ...
      if (amount >= 1 + currentBid) {
        ...
        amountElement.classList.add("is-invalid")
        feedback.innerText = "You must bid at least NOK" + (currentBid + 1).toFixed(2) + "!"
        modalBidButton.removeAttribute('disabled', '');
      }
    });
  }
...
function dataListener() {
  ...
  db.collection("auction-live").doc("items").onSnapshot(function (doc) {
    ...
    for (key in data) {
      ...
      cb.innerHTML = "NOK " + numberWithCommas(currPound) + " [" + bidCount + " bid" + (bidCount != 1 ? "s" : "") + "]"
    }
  })
}
